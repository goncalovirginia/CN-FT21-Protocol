...
public interface ControlAlgorithm {
	int LOCAL = Node.LOCAL; // the number of the virtual loop back interface
	int UNKNOWN = Node.UNKNOWN; // an unknown interface - means do not know how
	int INFINITY = 60; // just a suggestion
	 ...
	 */
	int initialise(int now, int node_id, Node nodeObj, GlobalParameters parameters, Link[] links, int nint);
	
	/**
	 * A periodic clock interrupt.
	 * @param now the current virtual clock value
	 */
	void on_clock_tick(int now);
	 ...
	 */
	void on_link_up(int now, int iface);
	 ...
	 * @param iface interface id where this link is connected
	 */
	void on_link_down(int now, int iface);
	
	/**
	 * Signals a timeout event
	 * @param now the current virtual clock value
	 */
	void on_timeout(int now);
	 ...
	 */
	void on_receive(int now, Packet p, int iface);
